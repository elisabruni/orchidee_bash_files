...
DIR_DEFAULT_CONFIG_spinup="$DIRsimu/FR_SAFRAN_SPIN"
#======================#======================#======================
...
awk 'NR>1 {print $0}' "$CSV_FILE" | while IFS=',' read -r site_code country research_theme longitude latitude; do
  ...
  sed -i "35c LIMIT_WEST=  $(echo "$lon-0.2" | bc)" "${DIRsimu}/${site_code}_spinup/PARAM/run.def"
  sed -i "36c LIMIT_NORTH=  $(echo "$lat+0.2" | bc)" "${DIRsimu}/${site_code}_spinup/PARAM/run.def"
  sed -i "37c LIMIT_SOUTH=  $(echo "$lat-0.2" | bc)" "${DIRsimu}/${site_code}_spinup/PARAM/run.def"
  sed -i "38c LIMIT_EAST=  $(echo "$lon+0.2" | bc)" "${DIRsimu}/${site_code}_spinup/PARAM/run.def"
